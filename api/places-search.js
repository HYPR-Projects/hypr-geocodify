...
// ─── Rate limiting (shared state via Supabase) ──────────────────────────────
// Audit finding: the previous in-memory sliding window never triggered in
// production — Vercel fans invocations across instances, so no single
// container accumulates the threshold. Replaced with an atomic counter in
// Postgres (rate_limit_hit SECURITY DEFINER function, fixed 1-minute window
// keyed ip:epochMinute). Adds ~30-60ms per request, negligible next to the
// 300-600ms Google round-trip. FAIL-OPEN: if the counter call errors or times
// out, the request proceeds — availability over protection for an internal
// tool; the cost ceiling is still bounded by the front-end guard and logging.
...
async function checkRateLimit(req) {
  ...
  try {
    const resp = await fetch(`${SUPABASE_URL}/rest/v1/rpc/rate_limit_hit`, {
      method: 'POST',
      headers: {
        'apikey': SUPABASE_KEY,
      ...
      body: JSON.stringify({ p_key: key, p_limit: RATE_LIMIT_PER_MIN }),
      signal: AbortSignal.timeout(1500),
    });
    if (resp.ok) {
      const limited = await resp.json();
      if (limited === true) {
        return { limited: true, retryAfterSec: 60 - Math.floor((Date.now() / 1000) % 60) };
      }
    }
  } catch (e) {
    // Fail-open by design.
